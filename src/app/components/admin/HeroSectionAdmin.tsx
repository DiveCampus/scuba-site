...
export default function HeroSectionAdmin() {
  ...
  useEffect(() => {
    console.log("🧠 Hero state updated:", hero);
  }, [hero]);
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center min-h-screen px-6">
        ...
        {editing === "top_text" ? (
          <input
            value={hero.top_text}
            onChange={(e) =>
              setHero({ ...hero, top_text: e.target.value })
            }
        ...
        {editing === "description" ? (
          <textarea
            ...
            onBlur={() => setEditing(null)}
            className="mt-4 bg-black/40 p-2 rounded w-full max-w-xl text-center"
          />
        ) : (
          <p
            onClick={() => setEditing("description")}
            className="mt-4 text-white/70 max-w-2xl cursor-pointer"
          >
            {hero.description}
          </p>
        )}
        ...
        {/* PRICE */}
        <div className="mt-8 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl px-8 py-6">
          ...
          {editing === "old_price" ? (
            <input
              ...
              onBlur={() => setEditing(null)}
              className="bg-black/40 text-white/50 text-center line-through"
            />
          ) : (
            <p
              onClick={() => setEditing("old_price")}
              className="text-sm text-white/50 line-through cursor-pointer"
            >
              AED {hero.old_price}
            </p>
          )}
          ...
          {editing === "price" ? (
            <input
              ...
              onBlur={() => setEditing(null)}
              className="bg-black/40 text-cyan-400 text-3xl text-center"
            />
          ) : (
            <p
            ...
            >
              {hero.price} <span className="text-cyan-400">AED</span>
            </p>
          )}
        </div>
        ...
        {editing === "cta_text" ? (
          ...
          <button
            onClick={() => setEditing("cta_text")}
            className="mt-8 px-8 py-3 bg-cyan-400 text-black rounded-lg"
          >
            {hero.cta_text}
          </button>
        )}
        ...
        <motion.button
          whileTap={{ scale: 0.95 }}
          onClick={handleSave}
          className="mt-6 px-6 py-3 bg-green-400 text-black rounded-xl"
        >
